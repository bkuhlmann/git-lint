...
RSpec.describe Git::Lint::Analyzers::CommitTrailerIssueValue do
  ...
  describe ".id" do
    it "answers class ID" do
      expect(described_class.id).to eq(:commit_trailer_issue_value)
    end
  end

  describe ".label" do
    it "answers class label" do
      expect(described_class.label).to eq("Commit Trailer Issue Value")
    end
  end
  ...
  describe "#valid?" do
    context "with valid key and value" do
      let :commit do
        Gitt::Models::Commit[
          body_lines: [],
          trailers: [Gitt::Models::Trailer.for("Issue: 123_abc-DEF")]
        ]
      end
